  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.isValid) {
        ...
        localStorage.setItem('bookflix-name', firstName + ' ' + lastName)
        navigate('/dashboard');
      } else {
        alert("Something went wrong signing up");
      }
      // Handle successful signup (e.g., redirect to dashboard or sign in)
    } catch (error) {
        ...
        <div className="w-full max-w-md">
          <div className="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-xl p-8 shadow-2xl">
            <div className="flex flex-col items-center gap-4 mb-8">
              ...
              <div className="text-center">
                <h2 className="text-3xl font-bold text-zinc-50 mb-2">Create Account</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <p className="text-center text-sm text-zinc-400">
              ...
              <Link to="/signin" className="text-red-600 hover:text-red-500 font-medium transition-colors">
                Sign in
              </Link>
            </p>
          </form>
